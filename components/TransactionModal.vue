<template>
  <UModal v-model="isOpen">
    <UCard>
      ...
      <UFormGroup label="Description" hint="Optional" name="description" class="mb-4" v-model="state.description">
        <UInput placeholder="Description" />
      </UFormGroup>
      <UFormGroup :required="true" label="Category" name="category" class="mb-4" v-if="state.type === 'Expense'">
        <USelect placeholder="Description" :options="categories" v-model="state.category"/>
      </UFormGroup>
...
const defaultSchema = z.object({
  ...
  amount: z.number().positive('Amount must to be more than 0')
})
